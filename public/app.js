...
const modal = {
    ...
    locationCombo : {
        select: function (event, ui) {
            ...
            $.ajax({
                ...
                success(response) {
                    modal.populateTechsCombo(response);
                },
                error(err) {
                    console.log('Error:', err);
                }
    ...
    techCombo : {
        select: function (event, ui) {
            ...
            $.ajax({
                ...
                type: 'GET',
                success(response) {
                    modal.populateJobTypesCombo(response);
                },
                error(err) {
                    console.log('Error:', err);
                }
    ...
    getTechs() {
        $.ajax({
            ...
            type: 'GET',
            data: {
                location_id: modal.data.P21_location_id
            },
            success(response) {
    ...
    populateTechsCombo(techs) {
        // console.log('Techs:', modal.data, techs);
        let select = $('#technician-edit').empty();
        techs.forEach(function (tech) {
            select.append(`<option value="${tech.value}">${tech.label}</option>`);
        });
        // this.getJobTypes();
    ...
    fillModalData() {
        ...
        location.find(`option[value="${modal.data.P21_location_id}"]`).attr('selected', true);
        location.combobox().combobox('destroy');
        location.combobox(modal.locationCombo);
        technician.find(`option`).removeAttr('selected');
        technician.find(`option[value="${modal.data.technician_id}"]`).attr('selected', true);
        technician.combobox().combobox('destroy');
        technician.combobox(modal.techCombo);
        jobType.find(`option`).removeAttr('selected');
        jobType.find(`option[value="${modal.data.job_type}"]`).attr('selected', true);
        jobType.combobox().combobox('destroy');
        jobType.combobox({});
        jobName.val(modal.data.job_name);
        comment.val(modal.data.job_comment);
